...
class Site {
  constructor () {
    this.name = 'KeepFriends';
    this.downloadLink = 'https://pt.keepfrds.com/download.php?id={ID}';
    this.url = 'https://pt.keepfrds.com/';
    this.id = 10;
  };
  ...
  async searchTorrent (keyword) {
    const torrentList = [];
    const document = await this._getDocument(`${this.index}torrents.php?notnewword=1&incldead=1&spstate=0&inclbookmarked=0&option-torrents=0&search=${encodeURIComponent(keyword)}&search_area=${keyword.match(/tt\d+/) ? 4 : 0}&search_mode=0`);
    ...
    for (const _torrent of torrents) {
      ...
      torrent.title = title ? (title.nextSibling.nodeValue || title.nextSibling.childNodes[0].nodeValue).trim() : '';
