...
class Rss {
  ...
  async _reseedTargets () {
    ...
    for (const key of keys) {
      ...
      if (!client || !client.maindata || !client.maindata.torrents) {
        if (this.reseedClients.indexOf(key) !== -1) logger.error('Rss', this.alias, '下载器', key, '不存在或不可用');
        continue;
      }
      clients.push(client);
    }
  ...
  _reseedSkipSameSize (torrent, targetTorrent) {
    for (const key of Object.keys(global.runningClient)) {
      ...
      for (const _torrent of client.maindata.torrents) {
        if (_torrent.hash === targetTorrent.hash) continue;
        if (+_torrent.size === +torrent.size) return true;
      }
    }
    return false;
  }
  ...
  async _rssReseed (torrent) {
    ...
    if (this.skipSameTorrent && this._reseedSkipSameSize(torrent, targetTorrent)) {
      ...
      await this.ntf.rejectTorrent(this._rss, targetClient, torrent, '拒绝原因: 跳过同大小种子');
      return false;
    }
    try {
      this.addCount += 1;
      ...
      await util.runRecord('INSERT INTO torrents (hash, name, size, rss_id, category, link, record_time, add_time, record_type, record_note) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [hash || torrent.hash, torrent.name, torrent.size, this.id, this.category, torrent.link, moment().unix(), moment().unix(), 1, '辅种']);
      logger.info(this.alias, '辅种成功:', torrent.name, '->', targetClient.alias);
      try {
        await this.ntf.addTorrent(this._rss, targetClient, torrent);
      } catch (e) {
        logger.error('通知信息发送失败: \n', e);
      }
      return true;
    } catch (error) {
      logger.error(this.alias, '辅种失败:', torrent.name, '\n', error);
      ...
      try {
        await this.ntf.addTorrentError(this._rss, targetClient, torrent);
      } catch (e) {
        logger.error('通知信息发送失败: \n', e);
      }
      return false;
  ...
  async rss (_torrents) {
    ...
    for (const torrent of torrents) {
      ...
      if (await this._rssReseed(torrent)) {
        continue;
      }
      if (this.onlyReseed) {
        continue;
